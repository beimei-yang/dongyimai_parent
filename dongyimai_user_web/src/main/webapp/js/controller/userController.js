...
app.controller('userController' ,function($scope,$controller   ,userService){	
	...
	$scope.reg = function(){
		...
		// 两次密码必须相同，但不可以为空
		if($scope.entity.password == "" || $scope.entity.password == null || $scope.entity.password != $scope.entity.repassword){
			alert("两次密码必须相同，且不可以为空");
			return ;
		}
		userService.add($scope.entity,$scope.code).success(
			function(response){
				if(response.success){
					alert("注册成功");
					// 跳转
					//location.href="http://localhost:9003/inde.html";
				}else{
